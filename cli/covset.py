...
type CompressionType = Literal["identity", "zlib", "bzip2"]
...
class EncodedCoverage(TypedDict):
    ...
    line. TODO clarify bit ordering
...
class FileInfo(TypedDict, total=False):
    ...
    config-inputs : ConfigReference
        Configuration parameters explicitly provided when compiling this file.
        Examples: preprocessor definitions like "-DDEBUG", "-DNDEBUG".
        Can be an index into "configs" or an inline array of strings.

    config-outputs : ConfigReference
        Full configuration environment the file was compiled with,
        including implicit compiler-added options. This represents the
        actual preprocessor state as best as can be determined.
        Can be an index into "configs" or an inline array of strings.
...
class FileInfoWithConfigInputs(FileInfo):
    """FileInfo variant with config-inputs field."""

    pass


class FileInfoWithConfigOutputs(FileInfo):
    """FileInfo variant with config-outputs field."""
...
def encode_bitmap(binary_data: bytes, compression: CompressionType = "zlib") -> EncodedCoverage:
...
def _bitwise_op(bitmap1, bitmap2, op):
    """
    Performs a bitwise operation (union, intersection, etc.) on two bitmaps.
    Pads the shorter bitmap with zeros to match the length of the longer one.
    """
    len1, len2 = len(bitmap1), len(bitmap2)
    if len1 < len2:
        bitmap1 += b"\x00" * (len2 - len1)
    elif len2 < len1:
        bitmap2 += b"\x00" * (len1 - len2)

    int1 = int.from_bytes(bitmap1, "big")
    int2 = int.from_bytes(bitmap2, "big")
    ...
    return result_int.to_bytes(max(len1, len2), "big")


def op_negate(covset, compression="zlib"):
    """Performs a bitwise NOT on all file bitmaps in a covset."""
    new_files = {}
    for fhash, info in covset.files.items():
        new_info = info.copy()
        bitmap_bytes = decode_bitmap(info["encodedcoverage"])

        if not bitmap_bytes:
            new_info["encodedcoverage"] = encode_bitmap(b"", compression)
            new_files[fhash] = new_info
            continue

        bit_length = len(bitmap_bytes) * 8
        mask = (1 << bit_length) - 1

        bitmap_int = int.from_bytes(bitmap_bytes, "big")

        negated_int = ~bitmap_int & mask

        # Ensure the byte length is consistent
        num_bytes = (bit_length + 7) // 8
        new_bitmap_bytes = negated_int.to_bytes(num_bytes, "big")

        new_info["encodedcoverage"] = encode_bitmap(new_bitmap_bytes, compression)
        new_files[fhash] = new_info
...
def show(covset_data: CovSetDict, codebase_path: Path):
    ...
    for fhash, file_info in files_to_visualize.items():
        ...
        try:
            bitmap_bytes = decode_bitmap(file_info["encodedcoverage"])
            ...
            with open(source_file_path, "r", encoding="utf-8") as f:
                for i, line in enumerate(f):
                    ...
                    if byte_index < len(bitmap_bytes):
                        ...
                        # Check the bit (assuming MSB is the first line in the byte)
