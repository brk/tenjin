...
2. Merge `master`, or some intermediate commit, into the `tenjin` branch of `Aarno-Labs/c2rust`. Assuming `tenjin` is the current branch, this is just `git merge <COMMIT>`, with a commit message of the form "Merge commit `COMMIT` ('master' as of YYYY-MM-DD) into tenjin".
Push after validating that `10j cargo +1.88.0 nextest run -p c2rust -p c2rust-transpile` still works.
