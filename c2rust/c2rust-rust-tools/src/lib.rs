...
fn run_rustc(
    ...
    for imported_crate in imported_crates {
        ...
        // For `{imported_crate}::*`.
        let relative_path_undec = match edition {
        ...
        let relative_path_unres = match edition {
            Edition2021 => format!("error[E0433]: failed to resolve: use of unresolved module or unlinked crate `{imported_crate}`"),
            Edition2024 => format!("error[E0433]: cannot find module or crate `{imported_crate}` in this scope"),
        };

        let absolute_path = absolute_path.as_str();
        let absolute_use_path = absolute_use_path.as_str();
        let relative_path_undec = relative_path_undec.as_str();
        let relative_path_unres = relative_path_unres.as_str();
        dbg!(absolute_path);
        dbg!(absolute_use_path);
        dbg!(relative_path_undec);
        dbg!(relative_path_unres);
        ...
        let relative_path_undec = error_lines.remove(relative_path_undec);
        let relative_path_unres = error_lines.remove(relative_path_unres);

        assert!(absolute_path || absolute_use_path || relative_path_undec || relative_path_unres);
    }
